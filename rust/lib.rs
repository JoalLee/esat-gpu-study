...
use numpy::{PyReadonlyArrayDyn, ToPyArray, PyArrayMethods};
use ndarray::{Array2};
use nalgebra::*;
...
mod backend;
#[path = "ls_nmf_batched.rs"]
mod batched;
use backend::select_device;
use batched::ls_nmf_batched;
...
fn ls_nmf<'py>(
    ...
    // Determine device: prefer CUDA (NVIDIA) or Metal (Apple Silicon)
    let prefer_gpu = prefer_gpu.unwrap_or(false);
    let (device, _device_name) = select_device(prefer_gpu);
    let use_gpu = prefer_gpu && !device.is_cpu();
    ...
    // Helper: create f32 tensor directly on the device
    let make_gpu_tensor = |data: Vec<f32>, rows: usize, cols: usize| -> candle_core::Result<Tensor> {
        Tensor::from_vec(data, (rows, cols), &device)
    };
    let result = if use_gpu {
        // Convert to Candle tensors: create f32 tensors directly on device
        let v_t = make_gpu_tensor(
            v_arr.iter().copied().collect::<Vec<f32>>(),
            v_arr.nrows(), v_arr.ncols(),
        ).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("{}", e)))?;
        let u_t = make_gpu_tensor(
            u_arr.iter().copied().collect::<Vec<f32>>(),
            u_arr.nrows(), u_arr.ncols(),
        ).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("{}", e)))?;
        let we_t = make_gpu_tensor(
            we_arr.iter().copied().collect::<Vec<f32>>(),
            we_arr.nrows(), we_arr.ncols(),
        ).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("{}", e)))?;
        let w_t = make_gpu_tensor(
            w_arr.iter().copied().collect::<Vec<f32>>(),
            w_arr.nrows(), w_arr.ncols(),
        ).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("{}", e)))?;
        let h_t = make_gpu_tensor(
            h_arr.iter().copied().collect::<Vec<f32>>(),
            h_arr.nrows(), h_arr.ncols(),
        ).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("{}", e)))?;

        // Call GPU update function
        ls_nmf_update_gpu(
...
fn ls_nmf_update_gpu<'py>(
    ...
    let wev = we.mul(&v).map_err(|e| format!("STEP wev=we.mul(v): {}", e))?;

    for i in 0..max_iter {
        // Update H
        if !hold_h || (delay_h > 0 && i > delay_h) {
            let wh = w.matmul(&h).map_err(|e| format!("STEP wh=w.matmul(h): {}", e))?;
            let h_num = w.t().map_err(|_| "STEP w.t()")?.matmul(&wev).map_err(|e| format!("STEP h_num: {}", e))?;
            let h_den = w.t().map_err(|_| "STEP w.t()")?.matmul(&we.mul(&wh).map_err(|e| format!("STEP we.mul(wh): {}", e))?).map_err(|e| format!("STEP h_den: {}", e))?;
            let h_delta = h_num.div(&h_den).map_err(|e| format!("STEP h_delta: {}", e))?;
            h = h.mul(&h_delta).map_err(|e| format!("STEP h=h.mul: {}", e))?;
        }

        // Update W
        let wh = w.matmul(&h).map_err(|e| format!("STEP wh2=w.matmul(h): {}", e))?;
        let w_num = wev.matmul(&h.t().map_err(|_| "STEP h.t()")?).map_err(|e| format!("STEP w_num: {}", e))?;
        let w_den = we.mul(&wh).map_err(|e| format!("STEP we.mul(wh2): {}", e))?.matmul(&h.t().map_err(|_| "STEP h.t2()")?).map_err(|e| format!("STEP w_den: {}", e))?;
        let w_delta = w_num.div(&w_den).map_err(|e| format!("STEP w_delta: {}", e))?;
        w = w.mul(&w_delta).map_err(|e| format!("STEP w=w.mul: {}", e))?;

        let qtrue = calculate_q_gpu(&v, &u, &w, &h).map_err(|e| format!("STEP calculate_q: {}", e))?;
        q = qtrue;
        q_list.push_back(q);
        q_list_full.push_back(q);
        converge_i = i;
        ...
        if now.duration_since(last_update) >= update_interval || converged || (i + 1) == max_iter {
            ...
            let qrobust = match calculate_q_robust_gpu(&v, &u, &w, &h, robust_alpha) {
                Ok(qr) => qr,
                Err(_) => qtrue, // fallback: use non-robust Q
            };
            let mse = qtrue / datapoints;
    ...
    // Convert final matrices back to Array2<f64>
    // Note: read f32 from device, then cast to f64 (to_dtype on Metal fails for f32->f64)
    let final_w = {
        let w_vec2 = w.to_vec2()?;
        let w_flat: Vec<f64> = w_vec2.into_iter().flatten().map(|x: f32| x as f64).collect();
        Array2::from_shape_vec(w.shape().dims2()?, w_flat)?
    };
    let final_h = {
        let h_vec2 = h.to_vec2()?;
        let h_flat: Vec<f64> = h_vec2.into_iter().flatten().map(|x: f32| x as f64).collect();
...
fn ws_nmf<'py>(
    ...
    // Determine device: prefer Metal (Apple Silicon) or CUDA (NVIDIA)
    let prefer_gpu = prefer_gpu.unwrap_or(false);
    let device = Device::Cpu;  // CPU with Accelerate BLAS (AMX on Apple Silicon)
    #[cfg(feature = "cuda")]
    let device = if prefer_gpu && device.is_cpu() {
        Device::cuda_if_available(0).unwrap_or(Device::Cpu)
    } else {
        device
    };
    let use_gpu = prefer_gpu;  // Use candle CPU path with Accelerate BLAS
...
fn ws_nmf_update_gpu<'py>(
    ...
    for i in 0..max_iter {
        ...
        // Update H (same multiplicative update as LS-NMF, matching CPU version)
        if !hold_h || (delay_h > 0 && i > delay_h) {
            let wh = w.matmul(&h)?;
...
fn calculate_q_robust_gpu(
    ...
    let robust_uncertainty = scaled_residuals
        .div(&Tensor::from_vec(vec![robust_alpha as f32], (1,), v.device())?)?
        .sqrt()?
        .mul(u)?;
    ...
    let mask = scaled_residuals.lt(&Tensor::from_vec(vec![robust_alpha as f32], (1,), v.device())?)?;
...
fn esat_rust(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(ls_nmf_batched, m)?)?;
