...
class BatchSA:
    ...
    def train(self, min_limit: int = None):
        ...
        # GPU batched path: no Manager needed (avoids fork/Metal crash and
        # multiprocessing spawn issues on macOS).
        if self.use_gpu and self.method == 'ls-nmf':
            logger.info("Running GPU batched LS-NMF.")
            all_sas = []
            for i in range(1, self.models + 1):
                ...
                # Use local variables to avoid mutating self.H/self.W in loop
                i_H = self.H
                if self.H is not None:
                    _H = np.array(self.H)
                    i_H = _H[i - 1] if _H.ndim == 3 else _H
                i_W = self.W
                if self.W is not None:
                    _W = np.array(self.W)
                    i_W = _W[i - 1] if _W.ndim == 3 else _W
            ...
            best_model_0 = -1
            best_q = float("inf")
            for i, sa in enumerate(all_sas):
                model_idx = i + 1
                sa.W = result['w'][i]
                sa.H = result['h'][i]
                sa.WH = np.matmul(sa.W, sa.H)
                sa.Qtrue = float(result['q'][i])
                sa.Qrobust, _ = qr_loss(V=sa.V, U=sa.U, W=sa.W, H=sa.H)
                sa.model_i = model_idx
                sa.converged = False
                sa.converge_steps = self.max_iter
                sa.q_list = [sa.Qtrue]
                sa.metadata = {
                ...
                if _nmf_q < best_q:
                    best_q = _nmf_q
                    best_model_0 = i  # 0-based
            self.best_model = best_model_0
            self.runtime = round(time.time() - t0, 2)
            logger.info(f"GPU batched training completed in {self.runtime}s. Best model: {best_model_0 + 1}.")
            if self.verbose:
                for i, sa in enumerate(self.results):
                    logger.info(f"Model: {i + 1}, "
                                f"Q(true): {sa.Qtrue:.4f}, "
                                f"MSE(true): {float(sa.Qtrue / self.V.size):.4f}, "
                                f"Q(robust): {float(sa.Qrobust):.4f}, "
                                f"MSE(robust): {float(sa.Qrobust / self.V.size):.4f}, "
                                f"Seed: {sa.seed}, "
                                f"Converged: {sa.converged}, Steps: {sa.converge_steps}/{self.max_iter}")
                logger.info(f"Results - Best Model: {best_model_0 + 1}, "
