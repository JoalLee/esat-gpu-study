...
class Displacement:
    ...
    def _batch_train_disp(self, tasks):
    ...
    def _increase_disp(self, batch: int = -1):
        ...
        for factor_i in tqdm(range(self.H.shape[0]), desc="Increasing value for factors", position=0, leave=True):
            factor_results = {}
            tasks = [] if self.use_gpu else None
            for feature_j in tqdm(self.features, desc=f"+ : Batch {batch}, Factor {factor_i+1} - Features", position=0, leave=True):
                ...
                for i in range(len(self.dQmax)):
                    ...
                    if self.use_gpu:
                        tasks.append({
                            "new_H": new_H.copy().astype(np.float64),
                            "factor_i": factor_i, "feature_j": feature_j,
                            "dQ_key": self.dQmax[i], "batch": batch,
                            "dQ": dQ, "new_value": new_value, "search_i": search_i,
                            "percent": percent, "conc": factor_conc_i,
                            "i_results": i_results, "factor_results": factor_results,
                        })
                    else:
                        disp_i_sa = SA(V=self.V, U=self.U, factors=self.sa.factors, method=self.sa.method,
                                       seed=self.sa.seed, verbose=False)
            ...
            if self.use_gpu and tasks:
                w_list, h_list, q_list = self._batch_train_disp(tasks)
                for idx, task in enumerate(tasks):
                    task["i_results"][task["dQ_key"]] = {
                        "dQ": task["dQ"], "value": task["new_value"],
                        "percent": task["percent"],
                        "search steps": task["search_i"],
                        "swap": compare_all_factors(h_list[idx], self.H),
                        "conc": task["conc"],
                        "Q_drop": self.base_Q - float(q_list[idx]),
                    }
                for task in tasks:
                    fkey = f"feature-{task['feature_j']}"
                    if fkey not in factor_results:
                        factor_results[fkey] = {}
                    factor_results[fkey].update(task["i_results"])
