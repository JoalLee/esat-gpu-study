...
class Bootstrap:
    ...
    def _train(self,
        ...
        if self.use_gpu:
            ...
            all_indices = []
            all_sas = []
            _H = copy.deepcopy(self.base_H) if keep_H else None
            for i in range(1, self.bootstrap_n + 1):
                ...
                if block:
                    bs_data, bs_uncertainty, bs_W, bs_index = self._block_resample(
                        data=_V, uncertainty=_U, W=_W, seed=sample_seed, overlapping=overlapping)
                else:
                    bs_data, bs_uncertainty, bs_W, bs_index = self._resample(
                        data=_V, uncertainty=_U, W=_W, seed=sample_seed)
                all_indices.append(bs_index)
                train_seed = self.base_seed if reuse_seed else sample_seed
                sa_i = SA(V=bs_data, U=bs_uncertainty, factors=self.factors,
                          method=self.sa.method, seed=train_seed, verbose=False)
                sa_i.initialize(H=copy.deepcopy(_H) if _H is not None else None)
                all_sas.append(sa_i)

            # Phase 2: stack into batched tensors
            V_batch = np.stack([sa.V.astype(np.float64) for sa in all_sas], axis=0)
            We_batch = np.stack([sa.We.astype(np.float64) for sa in all_sas], axis=0)
            ...
            for i, sa_i in enumerate(all_sas):
                sa_i.W = result['w'][i]
                sa_i.H = result['h'][i]
                sa_i.WH = np.matmul(sa_i.W, sa_i.H)
                sa_i.Qtrue = float(result['q'][i])
                ...
                bs_i_mapping = self.map_contributions(W1=sa_i.W, H1=sa_i.H,
                                                      W2=self.base_W, H2=self.base_H,
                                                      threshold=self.threshold)
                self.bs_results[model_i] = {
                    "model": sa_i,
                    "index": all_indices[i],
